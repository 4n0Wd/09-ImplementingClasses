...
class Point(object):
    ...
    def move_to(self, depx, depy):
        self.distance_travelled = self.distance_travelled + math.sqrt((depx - self.x) ** 2 + (depy - self.y) ** 2)
        self.x = depx
        self.y = depy
        self.moves = self.moves + 1

    def move_by(self, dx, dy):
        self.distance_travelled = self.distance_travelled + math.sqrt((dx ** 2 + dy ** 2))
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x - self.inix) ** 2 + (self.y - self.iniy) ** 2)

    def get_distance_traveled(self):
        return self.distance_travelled

    def closer_to(self, p2, p3):
        distance1 = self.get_distance_from(p2)
        distance2 = self.get_distance_from(p3)
